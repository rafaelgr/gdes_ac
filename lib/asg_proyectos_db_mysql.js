...
// simpre y cuando el proyecto no esté oculto
module.exports.getAsgProyectos2 = function(callback) {
...
module.exports.postAsgProyectoEvaluadorBuscar = function(buscador, callback) {
    ...
    sql += " LEFT JOIN trabajadores AS e ON e.trabajadorId = asgp.evaluadorId";
    sql += " WHERE p.ocultar = 0";
    sql += " AND t.trabajadorId IN (SELECT trabajadorId FROM evaluador_trabajador WHERE evaluadorId = ?)";
    sql = mysql.format(sql, buscador.trabajadorId);
    connection.query(sql, function(err, result) {
        if (err) {
            callback(err, null);
