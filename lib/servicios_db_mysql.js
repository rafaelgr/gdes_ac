...
function comprobarServicio(servicio){
	...
	comprobado = (comprobado && servicio.hasOwnProperty("nombre"));
	return comprobado;
}
...
// postServiciosBuscar
// lee todos los registros de la tabla servicios cuyo
// nombre contiene la cadena de búsqueda. Si la cadena es '*'
// devuelve todos los registros
module.exports.postServiciosBuscar = function (buscador, callback) {
    var connection = getConnection();
    var servicios = null;
    var sql = "SELECT * FROM servicios";
    if (buscador.nombre !== "*") {
        sql = "SELECT * FROM servicios WHERE nombre LIKE ?";
...
module.exports.deleteServicio = function(id, servicio, callback){
	...
	sql = mysql.format(sql, servicio.servicioId);
	connection.query(sql, function(err, result){
		if (err){
			callback(err);
			return;
