...
module.exports.getTrabajadoresEvaluador = function(req, res) {
    var q = req.query;
    ...
    //
    trabajadoresDb.getTrabajadoresEvaluador(q.id, function(err, trabajadores) {
        if (err) {
...
module.exports.postTrabajadoresBuscar = function (req, res){
    ...
    if (buscador == null) {
        ...
        trabajadoresDb.postTrabajadoresBuscarEvaluador(buscador, function (err, trabajadores) {
            if (err) {
                res.send(500, err.message);
            } else {
                res.json(trabajadores);
