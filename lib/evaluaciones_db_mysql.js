...
function comprobarEvaluacion(evaluacion){
    ...
    if (comprobado) {
        ...
        comprobado = (comprobado && evaluacion.conocimiento.hasOwnProperty("evaluacionId"));
    }
    comprobado = (comprobado && asgObjetivo.hasOwnProperty("dFecha"));
    comprobado = (comprobado && asgObjetivo.hasOwnProperty("hFecha"));
    comprobado = (comprobado && asgObjetivo.hasOwnProperty("observaciones"));
	return comprobado;
}
...
function fromDbtoJsEvaluaciones(evaluacionDb){
    var evaluacionJs = {
        evaluacionId: evaluacionDb.evaluacionId,
        asgProyecto: {
            asgProyectoId: evgaluacionDb.asgProyectoId,
        ...
        conocimiento: {
            ...
            catConocimiento: {
                catConocimientoId: evaluacionDb.catConocimientoId,
                nombre: evaluacion.ncatConocimiento
            }
        },
        dFecha: conocimientoDb.dFecha,
        hFecha: conocimientoDb.nFecha,
        observaciones: conocimientoDb.observaciones
    };
    return evaluacionJs;
}
...
function fromJstoDbEvaluacion(evaluacionJs){
    var evaluacionDb = {
        ...
        conocimientoId: evaluacionJs.conocimento.conocimientoId,
...
module.exports.getEvaluaciones = function(callback){
    ...
    sql += " cc.catConocimientoId, cc.nombre AS ncatConocimiento";
    ...
    sql += " LEFT JOIN catconocimientos AS cc ON cc.catConocimientoId = c.catConocimientoId";
...
module.exports.postEvaluacion = function (evaluacion, callback){
	...
	evalaucionDb.evaluacionId = 0; // fuerza el uso de autoincremento
	sql = "INSERT INTO evaluaciones SET ?";
	sql = mysql.format(sql, evalucionDb);
	connection.query(sql, function(err, result){
		if (err){
            callback(err);
            return;
...
module.exports.putEvaluacion = function(id, evaluacion, callback){
	...
	sql = mysql.format(sql, [evaluacionDb, evalucionDb.evaluacionId]);
	connection.query(sql, function(err, result){
		if (err){
            callback(err);
            return;
