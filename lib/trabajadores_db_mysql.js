...
// getTrabajadoresEvaluador
// obtener todos los trabajadores a los que registra un evaluador
// un determinado evaluador (id --> evaluadorId)
module.exports.getTrabajadoresEvaluador = function(id, callback) {
    var connection = getConnection();
    var proyectos = null;
    var sql = "SELECT DISTINCT t.trabajadorId, t.nombre";
    sql += " FROM asg_proyectos AS a";
    sql += " LEFT JOIN trabajadores AS t ON  t.trabajadorId = a.trabajadorId";
    sql += " LEFT JOIN proyectos AS p ON p.proyectoId = a.proyectoId";
    sql += " WHERE a.evaluadorId = ?";
    sql += " AND p.ocultar = 0";
...
// postTrabajadoresBuscarEvaluador
// obtienen los trabajadores que son evaluados por el 
// trabbajador con el id pasado
module.exports.postTrabajadoresBuscarEvaluador = function(buscador, callback) {
    var connection = getConnection();
    var trabajadores = null;
    var sql = "SELECT DISTINCT t.trabajadorId, t.nombre";
    sql += " FROM asg_proyectos AS ap";
    sql += " LEFT JOIN trabajadores AS t ON t.trabajadorId = ap.trabajadorId";
    sql += " WHERE ap.evaluadorId = ?";
    sql += " ORDER BY 2";
